  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      *is* `Application.get_env/3`.
      ...
      # `Ambient.Value` stores under the raw term, which would make
      # `put_override([:port], v)` a different row from `put(:port, v)` – and
      # this module's own `overridden?/1` and `revert/1` normalize, so they
      # would report and act on the wrong row. Normalize here too, so
      # `[:port]` and `:port` are interchangeable across the whole generated
      # surface as the `key` typedoc promises.
      # No @spec here: `Ambient.Value` already declared one for this function,
      # and a second (narrower) one makes an overloaded contract with
      # overlapping domains, which dialyzer on Elixir 1.15 rejects outright.
      # `term()` covers `Ambient.Config.key()` anyway.
      def put_override(key, value) do
        Ambient.ProcessOverride.put(@ambient_table, Ambient.Config.__normalize__(key), value)
      end

      def delete_override(key) do
        Ambient.ProcessOverride.delete(@ambient_table, Ambient.Config.__normalize__(key))
      end
