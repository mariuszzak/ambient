defmodule Ambient.ConfigTest do
  ...
  describe "nested keys" do
    ...
    test "the raw writers normalize too, so the module can't lie about its own rows" do
      # Regression: `put_override/2` and `delete_override/1` come from
      # Ambient.Value and stored under the raw term, so `put_override([:k], v)`
      # wrote a row that this module's own `overridden?/1` and `revert/1` –
      # which normalize – could neither see nor clear.
      key = unique_key()
      put_app_env(key, "app-value")

      Config.put_override([key], "override")
      assert Config.get(key) == "override"
      assert Config.overridden?([key])
      assert Config.overridden?(key)

      assert :ok = Config.revert([key])
      refute Config.overridden?(key)
      assert Config.get(key) == "app-value"
      assert Ambient.ProcessOverride.fetch(Config.__ambient_table__(), key) == :error
      assert Ambient.ProcessOverride.fetch(Config.__ambient_table__(), [key]) == :error
    end
